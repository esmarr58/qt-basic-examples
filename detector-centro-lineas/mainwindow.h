...
private slots:
    ...
    cv::Mat detectarLineasBlancas(const cv::Mat &imagenBGR, int nivelDeteccion);
    cv::Mat detectarCentroEntreLineas(const cv::Mat& imagenBinaria);
    cv::Mat combinarImagenes(const cv::Mat &imagenOriginal, const cv::Mat &imagenResultado);
private:
    Ui::MainWindow *ui;
